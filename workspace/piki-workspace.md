...
- **다른 세션이 열어 둔 워크트리는 경고를 보고 고른다.** 후보 열거(`$HOME/.claude/scripts/piki-worktrees.sh`)가 `open` 으로 표시하고 `/pr` 이 뒤로 밀어 라벨을 붙일 뿐, 제외하지는 않는다. 같은 파일을 동시에 고칠 위험을 감수할지는 사용자가 정한다.

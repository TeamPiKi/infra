...
블록은 값을 담지 않는다: `interval`·`attempts` 는 default 없이 필수 인자다. default 를
두면 특정 서비스 값과 우연히 겹쳐(예: server 5x60) 명시 세팅을 빠뜨려도 조용히 통과한다.
값의 출처는 **호출부(서비스별 배포 설정) 한 곳**이며, sh 도 이 문서도 값을 소유하지 않는다.

## 서비스별 파라미터 (실측 현황 — non-authoritative)

아래 표는 **배선 전 실측 스냅샷**이다. 지금은 값을 소비하는 호출부가 없어 이 표가 유일한
기록처지만, 배선되는 순간 값의 SSOT 는 **서비스별 배포 설정(호출부)** 으로 옮겨가고, 이 표는
그 설정을 가리키는 참조가 된다. 어느 시점에도 값이 두 곳에 독립적으로 박히지 않는다
(값이 바뀌면 서비스별 설정 한 곳만 고친다).
...
아래 값은 사용법을 보이기 위한 예시일 뿐 authoritative 하지 않다. 실제 배선은 위 원칙대로
서비스별 배포 설정에서 값을 주입한다.

...
- **자기가 만들지 않은 워크트리는 제거하지 않는다.** 워크스페이스 루트 세션은 `EnterWorktree({path})` 로 **이미 있는** 워크트리에 들어가 작업한다(정상 경로). 그 경우 `ExitWorktree({action:"remove"})` 가 비소유자로 거부하는데, 그 거부가 곧 "내가 만든 자리가 아니다" 라는 신호다. 거부를 우회해 강제로 지우지 않는다 — 그 자리는 다른 세션이나 다음 작업이 쓰고 있을 수 있고, 이 스킬은 그것을 판단할 근거가 없다.
...
- 둘 다 통과(clean + 머지됨) → **제거한다**:
  ...
  4. **거부하면 그 사유를 읽고 갈린다.** 두 거부는 뜻이 정반대라 같이 다루면 안 된다.
     ...
     - **"not the owner of the worktree" 거부** — 이 세션이 만든 워크트리가 아니라 `EnterWorktree({path})` 로 들어간 자리다(워크스페이스 루트 세션의 정상 경로). **여기서 멈춘다.** `ExitWorktree({action: "keep"})` 로 나온 뒤 사실 그대로 알린다: "이 세션이 만든 워크트리가 아니라 제거하지 않았다. 지우려면 `git -C <repo> worktree remove <경로> && git -C <repo> branch -D <브랜치>`". **아래 5번 fallback 을 타지 않는다** — 거부를 우회하는 것이 곧 남의 작업 공간을 지우는 경로다.

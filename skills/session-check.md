세션을 종료해도 되는지 **read-only로 점검**한다 — 기본은 현재 워크트리/브랜치만 훑고, 인자 `all` 을 주면 전체 워크트리·브랜치·PR을 함께 스윕한다. **점검 단계는 어떤 변경도 하지 않는다.** 커밋·푸시·브랜치 삭제·prune 은 직접 실행하지 않고, 무엇을 어떤 도구로 정리할지 **안내만** 한다 (실제 정리는 `/commit`·`git push` 등으로 사용자가 한다). close 도 이 스킬이 실행하지 않는다. 닫아도 안전하고 현재 워크트리가 제거 대상이면 `/session-close` 입력을 한 줄 안내한다 (아래 `## 절차` 6).
...
- **점검은 read-only — 읽기·안내만, 변경 없음.** session-check 는 상태를 **읽기만** 한다. 커밋·푸시·브랜치 삭제·prune 을 직접 하지 않고, "무엇을 어떤 도구(`/commit`·`git push`·`git branch -D`)로 정리하라"고 안내한다. close 도 같다: `/session-close` 는 사용자만 입력할 수 있는 스킬이라(frontmatter `disable-model-invocation`) 이 스킬이 대신 부르지 못하고, 부르지 않는다.
- **`AskUserQuestion` 을 띄우지 않는다.** 정리는 전부 안내로 끝내고 "무엇을 처리할지" 사용자에게 고르게 하지 않는다. 리포트를 읽고 무엇을 할지는 사용자가 정한다.
...
- **base 를 하드코딩하지 않는다** — 소비 repo 마다 PR base 가 다르다(core 는 `dev`, extractor·renderer 는 `main`). `pr`·`issue` 스킬과 같은 우선순위로 파생한다: `origin/dev` 가 있으면 `dev`, 없으면 레포 default branch, 그것도 못 구하면 `main`. 엉뚱한 base 로 비교하면 남의 커밋까지 섞여 diff 가 부풀려진다.
...
- 점검에서 **정리 안내** 로 표시된 항목은, 최종 리포트에 "무엇을 어떤 도구로 정리하면 되는지"를 한 줄씩 적어준다. 이들 항목에는 어떤 변경도 실행하지 않는다.
...
4. 정리가 필요한 항목은 리포트에 "무엇을 어떤 도구로 정리할지" 함께 적는다.
5. 한 줄 결론(닫아도 안전 / 남은 주의 N건)으로 마무리한다.
6. 결론이 "닫아도 안전"이고 현재 워크트리가 **제거 대상 sub-worktree**(위에서 구한 `CUR` != `MAIN`, 즉 메인 체크아웃이 아닌 linked worktree)이면서 머지+clean 이면, 리포트 끝에 `/session-close` 를 입력하라고 한 줄 안내한다. `/session-close` 는 사용자만 입력할 수 있는 스킬이라 이 스킬이 호출하지 않는다. 메인 체크아웃이거나 제거 대상 워크트리가 없으면, 또는 주의가 남아 있으면 이 안내를 생략한다 (먼저 정리가 우선이다).

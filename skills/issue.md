...
**작업 공간은 워크트리를 기본으로 한다.** 일반 이슈의 브랜치를 만들 때 현재 체크아웃을 그 브랜치로 끌고 가는 대신, `.claude/worktrees/` 에 격리된 워크트리로 분리하는 것을 추천(첫 번째 옵션)한다. 메인 체크아웃이 작업 중 외부에서 바뀌어도 격리되고 core CLAUDE.md "별도 작업은 worktree 로 분리" 정책과 맞다. 사용자가 현재 브랜치 작업을 고를 수 있게 묻되 default 는 워크트리. Epic 은 브랜치 자체가 없어 해당 없음.
...
- **제목** (한 줄, 개조식 규약 "제목" 절의 `대상 + 동작명사` 형식, 40자 권장·70자 상한, 슬러그 의미 보존)
- **왜** (배경, 필요성)
- **무엇을** (목표 상태, 예상 범위)

분해 룰 (다듬기 통합):
- 문체·뎁스·라벨은 개조식 규약(`.claude/rules/writing-style.md`)을 따른다. `## 왜`·`## 무엇을` 아래는 항목마다 `###`, 그 아래 불릿, 라벨은 굵게. 단답은 그 구조로 펼치되 문장으로 늘리지 않는다. 파일이 없으면 infra 정본을 읽는다: `gh api -H "Accept: application/vnd.github.raw" repos/TeamPiKi/infra/contents/conventions/writing.md`. 규약 없이 본문을 쓰지 않는다
...
1. GitHub 브랜치 생성 + 이슈 연결. **`--checkout` 을 주지 않는다** — 현재 디렉토리를 끌고 가면 같은 브랜치를 워크트리에서 다시 체크아웃할 수 없어 충돌한다.
   ```bash
   # base 는 /pr 과 같은 우선순위로 파생 — origin/dev 가 있으면 dev, 없으면 repo default branch.
   # (dev 는 core 만 있고 다른 소비 repo 는 main 이 base 다. 하드코딩하면 core 밖에서 어긋난다.)
   if git rev-parse --verify origin/dev >/dev/null 2>&1; then BASE=dev; else BASE=$(gh repo view --json defaultBranchRef --jq '.defaultBranchRef.name'); fi
...
3. 세션을 워크트리로 진입시킨다 — `EnterWorktree` 도구를 **`path=".claude/worktrees/{slug}"`** 로 호출 (이미 만든 워크트리에 진입). `name=` 으로 새로 만들지 않는다 — 브랜치는 `gh issue develop` 이 위에서 파생한 base 로 이미 만들었고, `EnterWorktree(name=...)` 의 baseRef 기본값(`fresh`=origin/default-branch)은 base 가 default branch 와 다른 repo(core: base `dev`, default `main`)에서 분기 정책과 어긋나기 때문. 이후 작업·커밋·`/pr` 은 이 워크트리에서 진행된다.

#### 현재 브랜치 선택 시 (기존 동작)

```bash
# base 파생은 워크트리 경로와 동일 (origin/dev → repo default branch)
if git rev-parse --verify origin/dev >/dev/null 2>&1; then BASE=dev; else BASE=$(gh repo view --json defaultBranchRef --jq '.defaultBranchRef.name'); fi
gh issue develop {이슈번호} \
  --base "$BASE" \
  --name "{prefix}/{이슈번호}-{slug}" \

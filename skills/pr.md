...
워크스페이스 루트(`piki/`)는 여러 repo 를 자식으로 두고 **세션을 시작하는 자리**일 뿐, 어느 repo 에도 서 있지 않다. 그런데 루트도 git repo 라 `git rev-parse` 로는 소비 repo 와 구분되지 않는다. 더 나쁜 것은 아래 0-A 가드가 **조용히 통과한다**는 점이다 — 루트는 origin 이 없어 `BASE_GUESS` 가 `main` 으로 떨어지고 루트 브랜치는 `workspace` 라, 둘이 달라서 "정상(작업 브랜치)" 으로 판정된다. 그대로 진행하면 아무 repo 도 아닌 곳에서 create 모드까지 간다.

```bash
# 로비 판별: toplevel 바로 밑에 origin 이 TeamPiKi/* 인 repo 가 하나라도 있으면 워크스페이스 루트다.
# (소비 repo 안에는 nested TeamPiKi repo 가 없어 오탐이 없다. 루트 자신의 origin 유무는 보지 않는다 —
#  origin 없음은 "아직 remote 를 안 붙인 소비 repo" 와 구분되지 않기 때문이다.)
...
```bash
~/.claude/scripts/piki-worktrees.sh    # repo <TAB> path <TAB> branch <TAB> 세션열림
```

- **후보가 있으면** `AskUserQuestion` (single-select) 으로 "어느 워크트리에서 `/pr` 을 이어갈까요?" 를 묻는다. 옵션 라벨은 `<repo> / <branch>`, 설명에 경로를 넣는다. **세션이 "열림" 인 워크트리는 옵션에서 제외한다** — 다른 세션이 그 자리에서 작업 중이라 두 세션이 같은 파일을 고치게 된다(전부 열림이면 그 사실을 알리고 멈춘다).
  - 선택 → `EnterWorktree` 를 `path={선택한 경로}` 로 호출한 뒤 **0단계를 처음부터 다시 시작**한다. 이제 cwd 가 그 워크트리라 `LOBBY=0` 으로 떨어진다.
  - 거부 → 멈춘다.
- **후보가 없으면** 올릴 작업이 어디에도 없다. 그 사실을 알리고 멈춘다 (작업할 워크트리를 먼저 만들어야 한다 — `/issue` 또는 `git -C <repo> worktree add`).
...
- Task 섹션은 core 의 Discord PR 봇이 읽으므로, 핵심 작업을 간결하게 요약
...
6. 확인 후 `gh pr edit --body-file /tmp/pr_body_$SLUG.md` 로 갱신 (1번과 같은 브랜치 경로 — 별도 bash 호출이라 `SLUG=$(basename "$(dirname "$(git rev-parse --path-format=absolute --git-common-dir)")")_$(git branch --show-current | tr '/' '_')` 를 다시 구한다). 제목 변경이 있으면 `--title "새 제목"` 추가.

...
**형식은 개조식 규약(`.claude/rules/writing-style.md`)이 주인이다.** 이 스킬은 절차와 "무엇을 담고 빼는가"만 정한다.
...
**작성 지침 (내용):**
- **형식은 개조식 규약(`.claude/rules/writing-style.md`)을 따른다.** 본문을 쓰기 전에 읽는다. 파일이 없으면 infra 정본을 읽는다: `gh api -H "Accept: application/vnd.github.raw" repos/TeamPiKi/infra/contents/conventions/writing.md`. 규약 없이 본문을 쓰지 않는다
- 대화에서 나온 고민, 왜 이 방식을 선택했는지, 어떤 대안을 검토했는지를 우선 반영
- diff에서만 보이는 기계적 변경 나열은 최소화
- **본문은 일상어.** 클래스·메서드·상수명을 모든 불릿에 박지 않는다. 식별자는 규약의 `구현` 라벨 한 곳에 모은다. 기준: 이 도메인 처음 보는 팀원이 표·첫 불릿만으로 "누가 무엇을" 을 이해하는가
- **이슈와의 중복 금지 — Situation·Task 는 이슈를 옮겨 적는 자리가 아니다.** 이슈가 이미 "왜(Situation)" 와 "무엇을(Task)" 을 담고 있고, 작성자가 같으므로 PR 은 거기서 **더 나아가거나 상세해진 것만** 적는다. 연관 이슈가 있으면 두 섹션 모두 `###` 아래 첫 불릿을 `- #{번호}` 링크로 두고, 그 아래에 이슈에 없는 내용만 덧붙인다 — Situation 은 착수 후 드러난 원인·제약·실측, Task 는 이슈의 "무엇을" 과 달라진 범위(`범위:` 라벨). 덧붙일 것이 없으면 링크 한 줄로 끝낸다. 요약이라도 이슈 문장을 다시 쓰지 않는다 — 같은 문장이 두 문서에 실리면 한쪽이 낡을 때 어긋나고, 독자는 링크 팝오버·Development 사이드바로 이슈를 바로 본다. 이슈가 없는 브랜치만 두 섹션을 본래 뜻대로 채운다.
...
### PR 제목 규칙
- **연관 이슈가 있으면 이슈 제목 그대로.** 이슈가 없을 때만 규약 "제목" 절대로 새로 짓는다

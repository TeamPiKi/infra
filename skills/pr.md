...
```markdown
## Situation
- 연관 이슈가 있으면 `#{번호}` 링크 한 줄 + 이슈에 없는 추가 내용만 (아래 작성 지침 "이슈와의 중복 금지")
- 이슈가 없으면: 이 작업이 필요했던 배경/문제 상황, 대화에서 논의된 동기나 맥락

## Task
- 연관 이슈가 있으면 `#{번호}` 링크 한 줄 + 이슈의 "무엇을" 과 달라진 범위만
...
**작성 지침:**
- **문체·뎁스·라벨은 개조식 규약(`.claude/rules/writing-style.md`)을 따른다.** 본문을 쓰기 전에 읽는다. 문장 없음, 접속사 없음, 명사구 종결, 뎁스 상한 5, 관계는 규약의 라벨 목록으로만. 파일이 없으면 infra 정본을 읽는다: `gh api -H "Accept: application/vnd.github.raw" repos/TeamPiKi/infra/contents/conventions/writing.md`. 규약 없이 본문을 쓰지 않는다

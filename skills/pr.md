브랜치에서 작업한 내용을 STAR 구조 PR로 정리하여 GitHub에 올립니다. 이미 PR이 있으면 본문 STAR 를 최종 상태로 직접 갱신합니다 (살아있는 본문 — `### 3-B`; 증분 원장은 본문이 아니라 PR 커밋 탭·본문 edit history 가 담당). assignee(`@me`) · 라벨(연관 이슈에서 복사, 없으면 브랜치 prefix) · Project(1) 도 자동 설정합니다.
...
## Situation
### {항목}
- #{번호}     ← 연관 이슈가 있으면 첫 불릿. 그 아래엔 이슈에 없는 추가 내용만 (작성 지침 "이슈와의 중복 금지")
- {이슈가 없으면: 이 작업이 필요했던 배경/문제 상황, 대화에서 논의된 동기나 맥락}

## Task
### {항목}
- #{번호}     ← 연관 이슈가 있으면 첫 불릿. 그 아래엔 `**범위:**` 만
- {이슈가 없으면: 해결하려 한 핵심 과제, 대화에서 중요하게 다뤘던 고민이나 결정 포인트}
...
| 선택지 | 장점 | 비용 |     ← 대안이 둘 이상이면 불릿 대신 비교 표. 표는 불릿과 같은 층
|---|---|---|

## Result
### {항목: 변경의 결과/효과 (CI 가 보증하는 "테스트 통과" 류 자명한 사실은 적지 않음)}
- **위험:** {남는 리스크}
### 후속
- {뒤따를 작업}
...
**작성 지침:**
- **문체·뎁스·라벨은 개조식 규약(`.claude/rules/writing-style.md`)을 따른다.** 본문을 쓰기 전에 읽는다. 문장 없음, 접속사 없음, 명사구 종결, 뎁스 상한 5, `##` 아래는 항목마다 `###`, 관계는 규약의 라벨 목록으로만(굵게). 파일이 없으면 infra 정본을 읽는다: `gh api -H "Accept: application/vnd.github.raw" repos/TeamPiKi/infra/contents/conventions/writing.md`. 규약 없이 본문을 쓰지 않는다
- 각 섹션은 bullet point(`-`)로 작성
- 대화에서 나온 고민, 왜 이 방식을 선택했는지, 어떤 대안을 검토했는지를 우선 반영
- diff에서만 보이는 기계적 변경 나열은 최소화
- **사람 말로 먼저, 코드 이름은 최소.** 클래스·메서드·상수명을 모든 불릿에 박지 않는다. 본문은 일상어로 "무엇을·왜" 를 말하고, 꼭 필요한 식별자는 Action 의 `구현:` 라벨 불릿 한 곳에 모은다. 기준: 이 도메인 처음 보는 팀원이 표·첫 불릿만으로 "누가 무엇을" 을 이해하는가. (정밀도를 버리는 게 아니라 자리를 옮기는 것이다. 코드 디테일을 없애지는 않는다.)
...
- **한 불릿 한 정보.** 절을 잇지 말고 형제 불릿으로 분해한다. 부연만 뎁스로.
- **이슈와의 중복 금지 — Situation·Task 는 이슈를 옮겨 적는 자리가 아니다.** 이슈가 이미 "왜(Situation)" 와 "무엇을(Task)" 을 담고 있고, 작성자가 같으므로 PR 은 거기서 **더 나아가거나 상세해진 것만** 적는다. 연관 이슈가 있으면 두 섹션 모두 첫 줄을 `- #{번호}` 링크로 두고, 그 아래에 이슈에 없는 내용만 덧붙인다 — Situation 은 착수 후 드러난 원인·제약·실측, Task 는 이슈의 "무엇을" 과 달라진 범위(`범위:` 라벨). 덧붙일 것이 없으면 링크 한 줄로 끝낸다. 요약이라도 이슈 문장을 다시 쓰지 않는다 — 같은 문장이 두 문서에 실리면 한쪽이 낡을 때 어긋나고, 독자는 링크 팝오버·Development 사이드바로 이슈를 바로 본다. 이슈가 없는 브랜치만 두 섹션을 본래 뜻대로 채운다.
...
- Task 섹션은 Discord PR 봇이 요약으로 읽으므로 간결하게. 봇은 연관 이슈의 제목·상태를 별도 줄로 붙이므로, 이슈 링크만 남긴 Task 도 정보 손실이 없다
- 한국어로 작성, 기술 용어는 영어 허용
- **제목·본문에 이모지·물결(`~`)·em dash(`—`)를 쓰지 않는다.** 이모지는 렌더링 환경에 따라 깨지고 미적 선호에도 어긋난다. `~text~` 는 GitHub-flavored markdown 이 취소선(strikethrough)으로 렌더링해 두 물결 사이 텍스트를 통째로 줄 그어버린다. em dash 는 가독성 선호상 쓰지 않는다. 대체: 곁가지·부연은 아래 뎁스나 형제 불릿으로, approximately 는 "약", 범위는 "에서"나 하이픈(`-`)으로 표현한다.
...
**섹션 안 구조 (규약의 "구조" 절):**
- `##` 섹션 아래 항목마다 `###`, 그 아래 불릿. 항목이 하나뿐이어도 `###`. 불릿 0개 허용.
- `###` 제목은 작업 성격에 따라 자유롭게 — 매번 같은 이름을 쓸 필요 없음. 이번 PR 이 다룬 결을 가장 잘 드러내는 명사구로.
- 한 항목 안에서도 대상·조건이 갈리는 변경(엔드포인트별 응답, 케이스별 분기 등)은 불릿 나열 대신 표로 묶는다 — "대조는 표로" 를 적용한다.
...
### 본문 확인 — 마크다운 링크로 보여주기 (create · update 공통)

본문 초안을 채팅에 길게 펼치지 않고, 저장한 본문 파일의 **마크다운 링크를 채팅에 남겨** 사용자가 **직접 열어 보고 편집**하게 한다. Claude Code 는 채팅의 파일 링크를 cmd+클릭으로 에디터에서 바로 열어 주므로, `idea` 등으로 GUI 를 직접 띄우지 않는다. 임시 파일이 single source 이므로 사용자가 열어서 고친 내용이 그대로 `--body-file` 로 올라간다 (채팅 copy-paste 왕복 제거, 에디터 마크다운 프리뷰로 표가 표처럼 보임).

채팅에는 제목 한 줄과 함께 링크만 남긴다:

```markdown
[PR 본문 확인·편집](/tmp/pr_body_{슬러그}.md)
```

- 링크를 남긴 뒤 **확인 게이트를 `AskUserQuestion` 번호 선택으로 띄운다.** 자유 텍스트로 "검토 후 진행하라고 하세요" 라고 묻지 않는다. 사용자가 매번 답을 타이핑하지 않고 번호로 고르게 한다. 채팅엔 제목 한 줄과 본문 링크만 짧게 남기고, 다음 두 선택지를 `AskUserQuestion` 으로 묻는다. **첫 옵션의 동사는 0-B 에서 이미 결정된 모드를 그대로 따른다 — create 면 "올린다", update 면 "갱신한다". 모드는 `gh pr view` 의 PR 존재 여부로 결정론적으로 갈리므로(주관 판단이 아니다) 실제 호출엔 그 한 단어만 넣는다. 슬래시로 둘 다 띄우지 않는다:**
...
3. **본문 STAR 를 최종 상태로 직접 고친다 (살아있는 본문).** 기준 한 줄: **"머지 후 이 본문만 읽은 독자가 최종 상태를 정확히 이해하는가."** 2단계 작성 지침(이슈와의 중복 금지 포함)은 갱신본에도 그대로 적용한다.
   ...
   - 큰 전환은 "처음 시도 → 전환한 흐름" 자체를 STAR 서사에 흡수한다 (`/commit` 본문 철학과 같은 결). 단, 전환의 종류에 따라 행선지가 갈린다:
...
### PR 제목 규칙
- **연관 이슈가 있으면 이슈 제목 그대로.** 이슈가 없을 때만 새로 짓는다
- 형식은 개조식 규약(`.claude/rules/writing-style.md`) "제목" 절: `대상 + 동작명사`, 동작 하나, 40자 권장·70자 상한

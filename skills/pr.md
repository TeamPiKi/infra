...
```markdown
## Situation
- 이 작업이 필요했던 배경/문제 상황 (연관 이슈가 있으면 한두 줄 요약에 그친다 — 아래 작성 지침)
- 이슈 이후 새로 안 사실, 대화에서 논의된 동기나 맥락
...
- **이슈와의 재진술 금지 — Situation 은 이슈를 옮겨 적는 자리가 아니다.** 연관 이슈가 있으면 이슈의 "왜" 는 한두 줄로 요약하고, 나머지는 이슈 이후 새로 안 사실(착수 후 드러난 원인·제약·실측)만 적는다. 이슈가 문제 진술을, PR 이 해법과 맥락을 맡는 분업이라 같은 문장이 두 문서에 실리면 한쪽이 낡을 때 어긋난다. 요약을 아예 생략하고 링크만 두지는 않는다 — 본문은 Discord 봇과 머지 후 히스토리에서 단독으로 읽힌다.

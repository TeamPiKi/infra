...
# 다르면 사람이 개입한 것으로 보고 그 세션은 영구 백오프한다(.off). 다시 켜려면 그 .off 파일을 지운다.
#
...
# 첫 프롬프트인데 이미 제목이 있으면 사용자가 직접 붙인 것이다 — 그 세션은 건드리지 않는다.
#
# session_title 은 사용자·훅이 지정한 제목(customTitle)을 담고, 첫 프롬프트 시점에는 우리가 아직
# 아무것도 방출하지 않았다. 그러니 여기 값이 있다는 건 `claude -n <이름>` 으로 이름을 주고 띄웠거나
# 그 전에 직접 지정했다는 뜻이다. (첫 프롬프트 요약으로 만들어지는 자동 제목은 이 필드가 아니다.)
#
# 아래 변경 감지만으로는 이 경우를 못 잡는다 — prev 가 비어 있어 비교가 통째로 건너뛰어지고,
# 결국 첫 방출이 사용자 이름을 덮어쓴다.
if [ "$n" -eq 1 ] && [ -n "$cur" ]; then
    : > "$off"
    exit 0
fi
...
# 세션마다 count·last·next·off 파일이 생기고 지우는 곳이 없어 무한히 쌓인다. 한 세션에 한 번(2번째
# 프롬프트, 아래 seed 와 같은 박자) 30일 넘게 손대지 않은 파일을 지운다. 백그라운드라 입력을 안 막는다.
if [ "$n" -eq 2 ]; then
